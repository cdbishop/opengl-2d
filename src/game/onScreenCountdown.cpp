...
OnScreenCountdown::OnScreenCountdown(TextManager::Ptr textManager, glm::vec2 position, std::string prefix)
...
void OnScreenCountdown::StartCountdown(unsigned int seconds, Callback cb)
{
  ...
  auto updateCallack = [this](unsigned int second, unsigned int index) {
    ...
    _countDown->_secondSprites[index] = _textManager->AddText(_prefix + std::to_string(second), _position);
  };
  ...
  _countDown->_secondSprites[0] = _textManager->AddText(_prefix + std::to_string(seconds), _position);
  _eventManager->After(std::chrono::seconds(seconds), [this]() {
    _textManager->RemoveText(_countDown->_secondSprites[_countDown->_secondSprites.size() - 1]);
