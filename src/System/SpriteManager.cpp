...
void SpriteManager::Remove(Sprite::Ptr sprite) {
  ...
  for (auto&[id, layer] : _sprites) {
    res = std::find_if(layer.begin(), layer.end(),
      [&](Sprite::Ptr spr) { return spr == sprite; });
...
void SpriteManager::RemoveFromLayer(Sprite::Ptr sprite, unsigned int layer) {
  auto res = std::find_if(_sprites[layer].begin(), _sprites[layer].end(),
    [&](Sprite::Ptr spr) { return spr == sprite; });
...
void SpriteManager::Render(Camera2D::Ptr camera) {
  ...
  for (auto&[id, layer] : _sprites) {
    for (auto&& sprite : layer) {
      ...
      model = glm::translate(
        model,
        glm::vec3(sprite->GetPosition() - camera->GetPosition(), 0.0f));
      ...
      model = glm::rotate(model, sprite->GetRotation(),
      ...
      _shader->SetUniformValue(
        "repeatFactorU", static_cast<float>(sprite->GetWidth()) /
